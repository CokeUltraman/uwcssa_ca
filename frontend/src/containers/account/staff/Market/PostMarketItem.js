...
const PostMarketItem = ({
  ...
  const { marketTypes, marketCategory } = useSelector(
  ...
  const uploadMarketItem = async () => {
    ...
    if (response.result) {
      history.push(
        `/marketItem/${response.response.data.createMarketItemInput.id}`
      );
    }
  };
  ...
  const uploadMarketType = async () => {
    ...
    const { name, owner } = marketTypeData;
